...
import { getWorkspaces, deleteWorkspace } from "@/actions";
import { usePathname, useRouter } from "next/navigation";
import { Separator } from "../ui/separator";
import { AddWorkspaceModal } from "../pages/workspace/add-workspace-modal";
import { DeleteConfirmDialog } from "../pages/workspace/delete-workspace-dialog";
...
export function AppSidebar({
  ...
  const handleDeleteWorkspace = (id: string, type: "PUBLIC" | "PRIVATE") => {
    setWorkspaceToDelete({ id, type });
  ...
  const confirmDeleteWorkspace = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await deleteWorkspace(workspaceToDelete.id);

      if (response.status === 200) {
        // Refresh the workspaces after deletion
        const res = await getWorkspaces();
        const userAndHisWorkspaces = res?.data;

        if (userAndHisWorkspaces) {
          ...
          // If the deleted workspace was the active one, redirect to the first available workspace
          if (workspaceToDelete.id === activeWorkspaceId) {
            const allWorkspaces = [
              ...userAndHisWorkspaces.workspaces,
              ...userAndHisWorkspaces.userWorkspaces.map((w) => w.workspace),
            ].filter(Boolean);
  ...
  // Only show delete for public workspaces the user owns
  const canDeleteWorkspace = (workspaceId: string) => {
    return myPublicWorkspaces.some((w) => w.id === workspaceId);
  };
    ...
    <>
      <Sidebar variant="inset">
        ...
        <SidebarContent>
          ...
          {/* Private Workspaces - No delete option */}
          <SidebarGroup>
          ...
          {/* Public Workspaces - Only show delete for owned workspaces */}
          <SidebarGroup>
            <Collapsible defaultOpen className="group/collapsible">
              <SidebarGroupLabel asChild>
                <CollapsibleTrigger className="flex w-full justify-between">
                  ...
                  <SidebarMenu>
                      ...
                      <SidebarMenuItem key={workspace.id}>
                        ...
                        {canDeleteWorkspace(workspace.id) && (
                          <SidebarMenuAction
                            showOnHover
                            onClick={() =>
                              handleDeleteWorkspace(workspace.id, "PUBLIC")
                            }
                    ...
                    {publicWorkspaces.map((workspaceDetails) => (
                      <SidebarMenuItem key={workspaceDetails.workspace.id}>
                        <SidebarMenuButton
                          isActive={
                            activeWorkspace.id === workspaceDetails.workspace.id
                          }
                          onClick={() => {
      ...
      <AddWorkspaceModal
        ...
        onSuccess={refreshWorkspaces} // Add this line
      />
      ...
      <DeleteConfirmDialog
        ...
        workspaceName={
          workspaceToDelete
            ? myPublicWorkspaces.find((w) => w.id === workspaceToDelete.id)
                ?.name || ""
            : ""
        }
      />
